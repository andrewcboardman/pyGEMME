...
def find_pkg():
	return pkg_resources(__name__)

def find_tools(name):
	return pkg_resources('tempfile', f'tools/{name}')

def edit_config(input_file, output_file, param_dict):
	with open(input_file,'r') as fid:
		input_config = [line.rstrip() for line in input_file]
	output_config = []
	...
	for line in input_config:
		...
		for key, value in param_dict.items():
			if line_[0] == key:
				change_line = True
				new_line = '\t\t'.join([line[0], value] + line[2:])
...
def JET_realign(
	...
	subprocess.call(f"sed -i 's/muscle\t\t")
	jet_conf_params = {
		'results':N_seqs_max
		'muscle'
	}
	edit_config(jet_conf_file, jet_conf_params)
	...
	jetcmd = ["java",
		"-Xmx1000m",
		"-cp gemme:gemme/jet/extLibs/vecmath.jar",
		"jet.JET",
