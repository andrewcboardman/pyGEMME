...
jet_file = args[4]
output_dir = args[5]
blosum62_file = args[6]
alphabet_file = args[7]
mutfile_does_not_exist = args[8]
mutfile = args[9]
...
# BLOSUM62  matrix
blosum62 = as.matrix(read.table(blosum62_file),row.names=1)
bgp = blosum62[order(rownames(blosum62)),order(colnames(blosum62))]
bg = apply(bgp,1,sum)

# Alphabet
alphabet = read.table(alphabet_file,sep="")$V1
...
res = list(
  computePSSM(ali,N[1], npos, bg, bgp, aa),
  computePSSM(aliCons,N[2], npos, bg, bgp, aa),
  computePSSM(aliVeryCons,N[3], npos, bg, bgp, aa)
  )
